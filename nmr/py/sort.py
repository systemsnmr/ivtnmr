...
## This is a cleaned-up copy of tt.py
## @Yar.Nikolaev. 2012-..

## More notes:
# - If modifying pulse-program names -- add those to the list!!
# - For auto-generation of cara repository, two fragments of final repository (`180917_INx_blank_1top.cara`, `180917_INx_blank_2bottom.cara`) need to be present in the py/user directory.
#==================================================
...
################################################################################
## Potentially editable user settings
################################################################################
...
hsqcPulprogs = ["hsqc15N.all", "sfhmqc01.yn"]
...
firstSpectrumId = 1
peaklist_id = 1
threshold = 20000 # 20000 - for new TopSpin4 data. Old settings: TOCSY ~50-200. HSQC ~7500 or 20000
...
def getTimeDifference(filename):
	"""
...
def timeToTitle(experiment):
	...
	if 0:
		...
		timeDiff = getTimeDifference(acqusFilePath)
...
def main():
    ...
    if flag_generate_cara_repo:
        ...
        short_name = re.findall('IN[A-Za-z0-9]+', NAME)[0]
