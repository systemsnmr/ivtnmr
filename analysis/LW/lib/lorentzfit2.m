function varargout = lorentzfit2(x,y,varargin)
% @YN 2016-05 edits
useAlternativeP0estimates = 0;
useStricterTolerance = 0;
useDefaultTolerance = 1;
...
%====================================================
% @YN: adjusted in lorentzfit2

if useAlternativeP0estimates
    [~, ymax_idx] = max(y(:));
    p3 = ((max(x(:))-min(x(:)))./10).^2;
    p2 = x(ymax_idx);
    p1 = max(y(:)-min(y(:))).*p3; % Amp*p3
    c = min(y(:));
else    
    p3 = ((max(x(:))-min(x(:)))./10).^2;
    p2 = (max(x(:))+min(x(:)))./2;
...
if useStricterTolerance
    optargs = {[],[],'3c',optimset('TolFun',max(mean(y(:))*1e-15,1e-15),'TolX',max(mean(x(:))*1e-15,1e-15))};
else
